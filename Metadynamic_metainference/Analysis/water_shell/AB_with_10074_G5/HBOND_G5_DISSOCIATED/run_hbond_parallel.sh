...
#To run this file in parallel do: cat dis | parallel ./run_hbond_parallel.sh
...
gmx_mpi select -s topol0.tpr -f ${PDB_DIR}/G5_${i}.pdb -os no.water_${i}.xvg -select 'resname SOL and within 0.4 of group "liga"' -n index_general_${i}.ndx -on no.water_${i}.ndx
...
shellhbonds=$((2*${shellshell} + ${shellbulk} + ${shelldrug}))
